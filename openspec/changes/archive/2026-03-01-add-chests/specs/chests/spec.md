...
When the player attempts to move onto a tile containing a closed chest, the chest SHALL open, play the `dungeon/chest-open` sound, and scatter its contents onto adjacent floor tiles. The player SHALL NOT move onto the chest tile.

#### Scenario: Open a closed chest
- **WHEN** the player walks into a tile containing a closed chest with contents [KEY, SWORD]
- **THEN** the chest SHALL become opened, the `dungeon/chest-open` sound SHALL play, and KEY and SWORD items SHALL appear on adjacent floor tiles
...
When a chest is opened, its contents SHALL be placed as item entities on walkable floor tiles adjacent to the chest in the 4 cardinal directions (up, down, left, right). If fewer adjacent floor tiles are available than items, excess items SHALL be added directly to the player's inventory.

#### Scenario: All adjacent tiles available
- **WHEN** a chest at (5, 5) opens with 3 items and tiles (5,4), (5,6), (4,5) are walkable floor
- **THEN** 3 items SHALL be placed on those adjacent floor tiles

#### Scenario: No adjacent floor tiles
- **WHEN** a chest opens with 2 items but all 4 adjacent tiles are walls or occupied
- **THEN** both items SHALL be added directly to the player's inventory
